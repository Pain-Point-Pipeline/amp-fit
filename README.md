...
3. Daniel grades before the session ends, within 72 hours at most: read, merge or close, append the row, fill the last five cells (`grade`, `fix`, `daniel_min`, `saved_min`, `limitation`). A row ungraded after 7 days counts as not useful.
   ...
   - `needs-decision`: grade the question, then answer it in that thread starting with `n=<next row>`; the continued run gets its own row. A thread that dies without a final message: write its row by hand with output `blocked` and grade `void`.
...
- Two consecutive `void` rows on one repo: run that repo's next pass with the local fallback; change nothing else.
...
- [ ] Pass 3, decided 2026-09-19, launch line: `amp-fit pass. n=3 repo=iamdanielkitchen/food-bridge-app`, sent as a continuation of the pass-2 thread `T-01a0b9fc-76e6-7386-9c5d-a709f99a98e5` (no `request=`: the first continuation pass).
