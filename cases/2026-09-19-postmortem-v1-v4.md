...
- The metric: one ledger row per pass, five cells filled by Daniel (grade, correction category, minutes spent grading, estimated minutes saved, one limitation phrase). Correction burden and value are both computed from those cells.
...
- Orb GitHub identity. Orb `git` and `gh` run as the Amp account's linked GitHub user, which is `ampfit2-lab` (author of PRs #27/#28); no Orb secrets exist. Daniel's private repos list only iamdanielkitchen as collaborator, so an Orb could not clone them. Fix: `ampfit2-lab` is added as a collaborator per repo (`../SETUP.md` step 2), which also separates author from grader.
